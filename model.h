...
static inline bool dit_version_is_dit1(DITVersion version) {
    if (version == VERSION_DIT1) {
        return true;
    }
    return false;
}
...
struct TensorStorage {
    ...
    void reverse_ne() {
        int64_t new_ne[DIT_MAX_DIMS] = {1, 1, 1, 1, 1};
        for (int i = 0; i < n_dims; i++) {
...
public:
    ...
    DITVersion get_dit_version();
    ggml_type get_dit_wtype();
