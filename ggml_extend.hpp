...
__STATIC_INLINE__ uint8_t* dit_tensor_to_image(struct ggml_tensor* input) {
...
__STATIC_INLINE__ uint8_t* dit_tensor_to_mul_image(struct ggml_tensor* input, int idx) {
    int64_t width    = input->ne[0];
    int64_t height   = input->ne[1];
    int64_t channels = input->ne[2];
    GGML_ASSERT(channels == 3 && input->type == GGML_TYPE_F32);
...
__STATIC_INLINE__ void dit_image_to_tensor(const uint8_t* image_data,
...
__STATIC_INLINE__ void dit_mask_to_tensor(const uint8_t* image_data,
                                         struct ggml_tensor* output,
                                         bool scale = true) {
    int64_t width    = output->ne[0];
    int64_t height   = output->ne[1];
...
__STATIC_INLINE__ void dit_apply_mask(struct ggml_tensor* image_data,
...
__STATIC_INLINE__ void dit_mul_images_to_tensor(const uint8_t* image_data,
                                               struct ggml_tensor* output,
                                               int idx,
                                               float* mean = NULL,
                                               float* std  = NULL) {
...
__STATIC_INLINE__ void dit_image_f32_to_tensor(const float* image_data,
                                              struct ggml_tensor* output,
                                              bool scale = true) {
    int64_t width    = output->ne[0];
    int64_t height   = output->ne[1];
...
// Tiling
__STATIC_INLINE__ void dit_tiling(ggml_tensor* input, ggml_tensor* output, const int scale, const int tile_size, const float tile_overlap_factor, on_tile_process on_processing) {
...
__STATIC_INLINE__ struct ggml_tensor* ggml_nn_attention(struct ggml_context* ctx,
                                                        ...
                                                        bool mask = false) {
#if defined(DIT_USE_FLASH_ATTENTION) && !defined(DIT_USE_CUDA) && !defined(DIT_USE_METAL) && !defined(DIT_USE_VULKAN) && !defined(DIT_USE_SYCL)
...
#if defined(DIT_USE_CUDA) || defined(DIT_USE_SYCL)
