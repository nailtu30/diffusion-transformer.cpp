...
void print_params(DITParams params) {
    printf("Option: \n");
    printf("    n_threads:         %d\n", params.n_threads);
    printf("    mode:              %s\n", modes_str[params.mode]);
    printf("    model_path:        %s\n", params.model_path.c_str());
    printf("    wtype:             %s\n", params.wtype < DIT_TYPE_COUNT ? dit_type_name(params.wtype) : "unspecified");
...
void print_usage(int argc, const char* argv[]) {
    ...
    printf("  -M, --mode [MODEL]                 run mode (class_label2img or convert, default: class_label)\n");
...
void parse_args(int argc, const char** argv, DITParams& params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-t" || arg == "--threads") {
            ...
            params.mode = (DITMode)mode_found;
            ...
            for (size_t i = 0; i < DIT_TYPE_COUNT; i++) {
                ...
                if (name == "f32" || trait->to_float && trait->type_size) {
                    ...
                    if (type == name) {
                        ...
                        params.wtype = (enum dit_type_t)i;
                        found        = true;
                        break;
                    }
                }
...
std::string get_image_params(DITParams params, int64_t seed) {
    std::string parameter_string = params.temp_prompt + "\n";
    parameter_string += "Steps: " + std::to_string(params.sample_steps) + ", ";
    parameter_string += "CFG scale: " + std::to_string(params.cfg_scale) + ", ";
    parameter_string += "Seed: " + std::to_string(seed) + ", ";
    parameter_string += "Size: " + std::to_string(params.width) + "x" + std::to_string(params.height) + ", ";
    parameter_string += "Model: " + dit_basename(params.model_path) + ", ";
    ...
    parameter_string += ", ";
    parameter_string += "Version: diffusion-transformer.cpp";
    return parameter_string;
}

/* Enables Printing the log level tag in color using ANSI escape codes */
void dit_log_cb(enum dit_log_level_t level, const char* log, void* data) {
    DITParams* params = (DITParams*)data;
    int tag_color;
    const char* level_str;
    FILE* out_stream = (level == DIT_LOG_ERROR) ? stderr : stdout;

    if (!log || (!params->verbose && level <= DIT_LOG_DEBUG)) {
        return;
    }
...
int main(int argc, const char* argv[]) {
    ...
    dit_set_log_callback(dit_log_cb, (void*)&params);
