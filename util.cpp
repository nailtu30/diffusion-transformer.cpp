...
static dit_progress_cb_t dit_progress_cb = NULL;
void* dit_progress_cb_data              = NULL;
...
static std::string dit_basename(const std::string& path) {
...
dit_image_t* preprocess_id_image(dit_image_t* img) {
    int shortest_edge   = 224;
    int size            = shortest_edge;
    dit_image_t* resized = NULL;
    ...
    resized = new dit_image_t{(uint32_t)shortest_edge,
...
void pretty_progress(int step, int steps, float time) {
    if (dit_progress_cb) {
        dit_progress_cb(step, steps, time, dit_progress_cb_data);
        return;
    }
    if (step == 0) {
        return;
...
static dit_log_cb_t dit_log_cb = NULL;
void* dit_log_cb_data         = NULL;

#define LOG_BUFFER_SIZE 1024

void log_printf(dit_log_level_t level, const char* file, int line, const char* format, ...) {
    va_list args;
    va_start(args, format);

    static char log_buffer[LOG_BUFFER_SIZE + 1];
    int written = snprintf(log_buffer, LOG_BUFFER_SIZE, "%s:%-4d - ", dit_basename(file).c_str(), line);
    ...
    if (dit_log_cb) {
        dit_log_cb(level, log_buffer, dit_log_cb_data);
    }

    va_end(args);
}

void dit_set_log_callback(dit_log_cb_t cb, void* data) {
    dit_log_cb      = cb;
    dit_log_cb_data = data;
}
void dit_set_progress_callback(dit_progress_cb_t cb, void* data) {
    dit_progress_cb      = cb;
    dit_progress_cb_data = data;
}
const char* dit_get_system_info() {
...
const char* dit_type_name(enum dit_type_t type) {
    return ggml_type_name((ggml_type)type);
}

dit_image_f32_t dit_image_t_to_dit_image_f32_t(dit_image_t image) {
    dit_image_f32_t converted_image;
...
dit_image_f32_t resize_dit_image_f32_t(dit_image_f32_t image, int target_width, int target_height) {
    dit_image_f32_t resized_image;
...
void normalize_dit_image_f32_t(dit_image_f32_t image, float means[3], float stds[3]) {
...
// Function to clip and preprocess dit_image_f32_t
dit_image_f32_t clip_preprocess(dit_image_f32_t image, int size) {
